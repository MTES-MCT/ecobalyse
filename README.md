...
<!-- il y a des variables sur Scalingo qui ne sont pas listées ici ; est-ce qu’elles sont encore utilisées ? SSH_KEY, *_IMPACTS_PATH, etc.-->
...
- `ECOBALYSE_DATA_DIR` : ?
<!-- Remplace sans doute DEFAULT_FROM_EMAIL ? Mais les deux n’ont pas la même valeur en prod -->
- `EMAIL_FROM`: ?
...
<!-- `development` est la valeur par defaut je pense, on devrait pouvoir juste laisser `npm install` -->
...
Il est possible de lancer la vérification du formatage à la main grâce à la commande suivante :
    ...
    $ npm run lint:all
    <!-- j’ai des erreurs, mais dans le repertoire .venv: il n’est pas exclu par défaut ? -->
