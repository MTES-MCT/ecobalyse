...
-}
materialRecycledWaste : Mass -> Mass -> Float -> Mass -> { mass : Mass, waste : Mass }
...
-}
makingWaste : Mass -> Float -> Mass -> { mass : Mass, waste : Mass }
...
-}
materialRecycledCo2 : Float -> Float -> Float -> Mass -> Float
materialRecycledCo2 pristineCC recycledCC recycledRatio baseMass =
    let
        ( recycledCo2, pristineCo2 ) =
            ( baseMass
                |> Quantity.multiplyBy recycledRatio
                |> Quantity.multiplyBy recycledCC
            , baseMass
                |> Quantity.multiplyBy (1 - recycledRatio)
                |> Quantity.multiplyBy pristineCC
            )
    in
