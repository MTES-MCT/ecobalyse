...
-}
genericWaste : Unit.QuantityVariationRatio -> Mass -> { mass : Mass, waste : Mass }
genericWaste qtyVariationRatio stageOutputMass =
    let
        stageInputMass =
    ...
    in
    -- We return the inputMass at this stage (considered as the output mass of the previous stage)
    { mass = stageInputMass, waste = Quantity.minus stageOutputMass stageInputMass }
...
makingImpacts impacts { countryElecProcess, countryHeatProcess, fadingProcess, makingComplexity } outputMass =
    -- Note: Fading, when enabled, is applied at the Making stage because
    -- it can only be applied on finished products (using stage output mass).
    -- Also:
