...
type alias Query2 =
    -- ?mass=1
    -- &growing.enabled=true
    -- &spinning.enabled=true&spinning.country=FR
    -- &fabric.enabled=true&fabric.country=FR&fabric.picking=1000&fabric.surfaceMass=12
    -- etc…
    { mass : Mass
    , materials : List MaterialQuery
    , product : Product.Id
    , growing : GrowingQuery
    , spinning : SpinningQuery
    , fabric : FabricQuery
    , dyeing : DyeingQuery
    , making : MakingQuery
...
decodeQuery2 : Decoder Query2
decodeQuery2 =
    Decode.succeed Query2
        ...
        |> Pipe.required "growing" decodeGrowingQuery
        |> Pipe.required "spinning" decodeSpinningQuery
        |> Pipe.required "fabric" decodeFabricQuery
        |> Pipe.required "dyeing" decodeDyeingQuery
        |> Pipe.required "making" decodeMakingQuery
        |> Pipe.required "distribution" decodeDistributionQuery
        |> Pipe.required "use" decodeUseQuery
        |> Pipe.required "eol" decodeEoLQuery
...
type alias GrowingQuery =
    { enabled : Bool
    }


decodeGrowingQuery : Decoder GrowingQuery
decodeGrowingQuery =
    Decode.succeed GrowingQuery
        |> Pipe.optional "enabled" Decode.bool True


type alias SpinningQuery =
    { enabled : Bool
    , country : Maybe Country.Code
    }


decodeSpinningQuery : Decoder SpinningQuery
decodeSpinningQuery =
    Decode.succeed SpinningQuery
        |> Pipe.optional "enabled" Decode.bool True
        |> Pipe.optional "country" (Decode.maybe Country.decodeCode) Nothing


type alias FabricQuery =
    { enabled : Bool
...
decodeFabricQuery : Decoder FabricQuery
decodeFabricQuery =
    Decode.succeed FabricQuery
        |> Pipe.optional "enabled" Decode.bool True
        |> Pipe.required "country" Country.decodeCode
...
type alias DyeingQuery =
    { enabled : Bool
    , country : Country.Code
    , weighting : Maybe Unit.Ratio
    }


decodeDyeingQuery : Decoder DyeingQuery
decodeDyeingQuery =
    Decode.succeed DyeingQuery
        |> Pipe.optional "enabled" Decode.bool True
        |> Pipe.required "country" Country.decodeCode
        |> Pipe.optional "dyeingWeighting" (Decode.maybe Unit.decodeRatio) Nothing


type alias MakingQuery =
    { enabled : Bool
    , country : Country.Code
    , airTransportRatio : Maybe Unit.Ratio
    , waste : Maybe Unit.Ratio
    }


decodeMakingQuery : Decoder MakingQuery
decodeMakingQuery =
    Decode.succeed MakingQuery
        |> Pipe.optional "enabled" Decode.bool True
        |> Pipe.required "country" Country.decodeCode
...
type alias DistributionQuery =
    { enabled : Bool
    }


decodeDistributionQuery : Decoder DistributionQuery
decodeDistributionQuery =
    Decode.succeed DistributionQuery
        |> Pipe.optional "enabled" Decode.bool True
...
decodeUseQuery : Decoder UseQuery
decodeUseQuery =
    Decode.succeed UseQuery
        |> Pipe.optional "enabled" Decode.bool True
...
decodeEoLQuery : Decoder EoLQuery
decodeEoLQuery =
    Decode.succeed EoLQuery
        |> Pipe.optional "enabled" Decode.bool True
