function createCSPDirectives(env) {
  const { MATOMO_HOST = "", POSTHOG_HOST = "", SENTRY_DSN = "" } = env;
  ...
  const AVATARS_GITHUB_HOST = "https://avatars.githubusercontent.com";
  const BLOB = "blob:";
  const DATA = "data:";
  ...
  return {
    ...
    "img-src": onlyValid([SELF, DATA, BLOB, AVATARS_GITHUB_HOST, GITHUB_RAW_HOST]),
    ...
    // OR we eventually switch to using Posthog only
