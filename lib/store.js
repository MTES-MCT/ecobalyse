    ...
    if (file) {
      ...
      reader.addEventListener("load", () => {
        try {
          ...
          importVersionBookmarks(results, stableVersionKey);
          importVersionBookmarks(results, ongoingVersionKey);
...
function importVersionBookmarks(results, key) {
  if (results) {
    ...
    if (Array.isArray(bookmarks) && bookmarks.length > 0) {
      ...
      localStorage.setItem(`${key}_backup`, previousStore);
