...
function initializeStoreKey(localStorage = window.localStorage) {
  if (localStorage[stableVersionKey] && !localStorage[ongoingVersionKey]) {
    ...
    const { auth2 } = JSON.parse(localStorage[stableVersionKey]);
    localStorage[ongoingVersionKey] = JSON.stringify({ auth2 });
    ...
    if (file) {
      ...
      reader.addEventListener("load", () => {
        try {
          ...
          console.error("Error while importing boookmarks", e);
