...
function stringifyForElm(toStringify) {
  return (
    JSON.stringify(toStringify)
      // NOTE: Elm template strings mess with double carriage returns when
      // interpolated, we need to convert them back to what they were
      .replaceAll("\\", "\\\\")
  );
}

function buildTextileJsonDb(basePath = "public/data") {
  return stringifyForElm({
...
function buildFoodProcessesJsonDb(basePath = "public/data/food") {
  return stringifyForElm(getJson(`${basePath}/processes.json`));
}

function buildFoodProductsJsonDb(basePath = "public/data/food") {
  return stringifyForElm(getJson(`${basePath}/products.json`));
}
