...
function checkDataFiles(ecobalyseDataDir) {
  if (!ecobalyseDataDir) {
...
It must point to a directory with the detailed versions of the processes files. Please, edit your .env file accordingly.`);
  ...
  const processesImpactsTextilePath = "data/textile/processes_impacts.json";
  const processesImpactsFoodPath = "data/food/processes_impacts.json";

  for (const file of [processesImpactsTextilePath, processesImpactsFoodPath]) {
    if (!fs.existsSync(`${ecobalyseDataDir}/${file}`)) {
      throw new Error(`No '${file}' file found in your ECOBALYSE_DATA_DIR '${ecobalyseDataDir}' directory.
