...
function getProcessesAsString(detailed = false) {
  return JSON.stringify(
    JSON.parse(
      fs.readFileSync(detailed ? dataFiles.detailed : dataFiles.noDetails, "utf8").toString(),
    ).concat(
      JSON.parse(
        fs.readFileSync(detailed ? dataFiles.genericDetailed : dataFiles.genericNoDetails, "utf8"),
      ),
    ),
  );
}
...
const dataFiles = {
  ...
  genericDetailed: "public/data/processes_generic_impacts.json",
  genericNoDetails: "public/data/processes_generic.json",
