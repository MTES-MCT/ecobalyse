...
function checkDataFiles() {
  ...
  if (!ECOBALYSE_DATA_DIR) {
    console.error(
      `
🚨 ERROR: You need to specify ECOBALYSE_DATA_DIR env variable.
...
`,
  ...
  for (const file of [processesImpactsTextilePath, processesImpactsFoodPath]) {
    if (!fs.existsSync(`${ECOBALYSE_DATA_DIR}/${file}`)) {
      console.error(
        `
  🚨 ERROR: No '${file}' file found in your ECOBALYSE_DATA_DIR '${ECOBALYSE_DATA_DIR}' directory.
     Be sure that the file structure inside '${ECOBALYSE_DATA_DIR}' is correct.
     -> Exiting the process.
  `,
      );
      process.exit(1);
