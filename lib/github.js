...
async function createExamplesPR(
  ...
  if (!examples) throw new Error("examples are missing");
  ...
  const {
    status,
    data: { html_url, diff_url, additions, deletions },
  } = await client.createPullRequest({
    owner,
    repo,
    head,
    title: `[${date}] Update ${type} product examples.`,
    body: `A few ${type} product example updates by **${name}**:\n\n> ${description}\n\nUne branche de recette sera déployée sous peu.`,
  ...
  return { status, html_url, diff_url, additions, deletions };
}
