...
def lci_catalog_url(process_id: str, lci_index: dict[str, str]) -> str | None:
    catalog_path = lci_index.get(process_id)
    return f"{GITHUB_LCI_CATALOG_URL}/{catalog_path}" if catalog_path else None
...
def check_hierarchy(
    ...
    """
    anomalies = []
    for variants in ingredient_by_base.values():
