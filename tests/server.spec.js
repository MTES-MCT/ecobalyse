...
describe("API server tests", () => {
  ...
  describe("/simulator/", () => {
    it("should validate the mass param", async () => {
      const response = await makeRequest("/simulator/");

      expect(response.statusCode).toBe(400);
      expect(response.body.error).toContain("La masse doit être supérieure ou égale à zéro.");
    });

    it("should validate the material param", async () => {
      const response = await makeRequest("/simulator/", "mass=0.170");

      expect(response.statusCode).toBe(400);
      expect(response.body.error).toContain("Impossible de récupérer la matière uuid=<empty>");
    });

    it("should validate the product param", async () => {
      const response = await makeRequest("/simulator/", "mass=0.170&material=f211bbdb-415c-46fd-be4d-ddf199575b44");

      expect(response.statusCode).toBe(400);
      expect(response.body.error).toContain("Produit non trouvé id=<empty>");
    });
    ...
    //   const testQueries = [{ ...successQuery, mass: "-1" }];
